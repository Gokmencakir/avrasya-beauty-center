...
İşletmenin logosu varsa (depoya yükle), **4 yere** konur:
- **Hakkımızda** bölümündeki büyük görsel alanına → **tam logo** (yazısıyla)
- **Menü (üst bar)** ve **footer**'daki ikon → logonun **ikon/monogram** kısmı (kare kırpım). Yanındaki `[İŞLETME ADI]` yazısı kalır. (CSS'te `.brand-logo` sınıfı hazır; `.crown` ikonunun yerine `<img class="brand-logo" ...>` gelir.)
- **Favicon** (tarayıcı sekmesi ikonu) → aynı monogram (`<link rel="icon" ...>`).
...
Logo varsa: tam halini Hakkımızda'ya koy; ikon/monogram kısmını kırpıp menü + footer
ikonuna ve favicon'a yerleştir. Sonra commit'leyip push et.
```
